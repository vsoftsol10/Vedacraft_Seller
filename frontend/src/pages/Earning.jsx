...
export default function Earning() {
  ...
  const loadEarnings = async () => {
    ...
    setError("");
    try {
      const response = await getEarnings("thisMonth");
  ...
  return <div>
    ...
    <div className="mb-5 grid grid-cols-1 gap-4 sm:grid-cols-2">
      <div className={STAT_CARD}><div className={STAT_ICON} style={{ background: "#d9f2df" }}><Package size={18} /></div><div><p className={STAT_LABEL}>Order Value</p><p className={STAT_VALUE}>{money(earnings?.orderValue)}</p></div></div>
      <div className={STAT_CARD}><div className={STAT_ICON} style={{ background: "#fdeacb" }}><ShoppingBag size={18} /></div><div><p className={STAT_LABEL}>Order Count</p><p className={STAT_VALUE}>{Number(earnings?.orderCount) || 0}</p></div></div>
    </div>
